...
func SaveOperaLog(message storage.Messager) (err error) {
	...
	var sysOperaLog SysOperaLog
	err = json.Unmarshal(rb, &sysOperaLog)
	if err != nil {
	...
	// 超出100个字符返回值截断
	if len(sysOperaLog.JsonResult) > 100 {
		sysOperaLog.JsonResult = sysOperaLog.JsonResult[:100]
	}

	err = db.Create(&sysOperaLog).Error
	if err != nil {
