...
// LevelForGorm convert to gorm log level
...
func GetLevel(level string) (Level, error) {
	...
	return InfoLevel, fmt.Errorf("Unknown Level String: '%s', defaulting to InfoLevel", level)
}
...
func V(level Level, log Logger) bool {
	l := DefaultLogger
	if log != nil {
		l = log
	}
	return l.Options().Level <= level
}
